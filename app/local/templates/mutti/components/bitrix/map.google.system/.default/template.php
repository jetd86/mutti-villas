            ...
            if (null == window.bGoogleMapScriptsLoaded) {
                if (window.google && window.google.maps) {
                    ...
                    BX.loadScript(
                        ...
                        function () {
                            if (BX.browser.IsIE())
                                setTimeout("window.google.load('maps', <?= intval(
                                    $arParams['GOOGLE_VERSION']
                                )?>, {callback: init_<?echo $arParams['MAP_ID']?>, other_params: 'language=<?=LANGUAGE_ID?>&key=" + apiKey + "'})", 1000);
                            else
                                google.load('maps', <?echo intval(
                                    $arParams['GOOGLE_VERSION']
                                )?>, {
                                    callback: init_<?echo $arParams['MAP_ID']?>,
                                    other_params: 'language=<?=LANGUAGE_ID?>&key=' + apiKey
                                });
                        }
                    );
                }
...
<div id="BX_GMAP_<?= $arParams['MAP_ID'] ?>" class="location-map bx-google-map">
    <?= GetMessage('MYS_LOADING' . (($arParams['WAIT_FOR_EVENT'] ?? null) ? '_WAIT' : '')); ?>
</div>
